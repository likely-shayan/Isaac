...
Curve2D::Curve2D(const CurveType2D &curveType_,
...
Curve2D::Curve2D(const Curve2D &curve) noexcept
    : Curve2D(curve.getCurveType(),
              curve.getPosition(),
              curve.getVelocity(),
              curve.getAcceleration()) {}
...
void Curve2D::setPosition(const Vector2d &newPosition) noexcept {
  position = newPosition;
}

Vector2d Curve2D::getVelocity() const noexcept { return velocity; }

void Curve2D::setVelocity(const Vector2d &newVelocity) noexcept {
  velocity = newVelocity;
}

Vector2d Curve2D::getAcceleration() const noexcept { return acceleration; }

void Curve2D::setAcceleration(const Vector2d &newAcceleration) noexcept {
