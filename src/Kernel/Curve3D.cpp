...
Curve3D::Curve3D(const CurveType3D &curveType_,
...
Curve3D::Curve3D(const Curve3D &curve) noexcept
    : Curve3D(curve.getCurveType(),
              curve.getPosition(),
              curve.getVelocity(),
...
void Curve3D::setPosition(const Vector3d &newPosition) noexcept {
  position = newPosition;
}

Vector3d Curve3D::getVelocity() const noexcept { return velocity; }

void Curve3D::setVelocity(const Vector3d &newVelocity) noexcept {
  velocity = newVelocity;
}

Vector3d Curve3D::getAcceleration() const noexcept { return acceleration; }

void Curve3D::setAcceleration(const Vector3d &newAcceleration) noexcept {
